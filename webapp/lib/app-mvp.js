...
function friendlyBleError(err) {
  ...
  if (name === 'SecurityError' || /secure context/i.test(msg)) {
    return "Web Bluetooth requires HTTPS. Open this page from getopenwhoop.pages.dev (not file:// or plain http://).";
  }
...
// Web Bluetooth only exists in Chrome/Edge (desktop + Android) and the native
// Capacitor build. iOS Safari/Chrome and Firefox have no navigator.bluetooth,
// so tell the user what to do instead of letting Connect silently no-op.
function bluetoothUnavailableMessage() {
  if (navigator.bluetooth) return null;
  const isIOS = /iP(hone|ad|od)/.test(navigator.userAgent) ||
    (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
  return isIOS
    ? "iOS browsers can't use Bluetooth. Open this page in the Bluefy app (App Store), or use Chrome/Edge on Android or a computer."
    : "This browser has no Web Bluetooth. Use Chrome or Edge on Android, Windows, Mac, or Linux.";
}

connectBtn.addEventListener('click', async () => {
  const unavailable = bluetoothUnavailableMessage();
  if (unavailable) { showError(unavailable); return; }
...
async function purgeDemoOnce() {
  const FLAG = 'openwhoopDemoPurgedV1';
