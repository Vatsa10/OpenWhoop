# openwhoop v0.3 (Browser PWA) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Migrate `openwhoop` from a Python recorder + HTTP dashboard to a pure-browser PWA that reads the Whoop 4.0 via Web Bluetooth, stores data in IndexedDB, and computes every metric in JavaScript. One codebase runs in Mac Chrome (dev) and iPhone Bluefy (target).
...
**Spec:** [`docs/superpowers/specs/2026-05-20-openwhoop-web-pwa-design.md`](../specs/2026-05-20-openwhoop-web-pwa-design.md)
...
openwhoop/                     (trimmed Python — kept for static-serve only)
...
#   openwhoop/db.py, metrics.py, sleep.py, zones.py, workouts.py, recorder.py, dashboard.py
...
Source: `openwhoop/db.py:15-110` (schema).

- [ ] **Step 1: Write `web/js/data/schema.js`** (single source of truth for stores + indexes)

```js
// IndexedDB schema for openwhoop v0.3.
// Mirrors the SQLite schema in openwhoop/db.py.

export const DB_NAME = 'openwhoop';
...
describe('openDb', () => {
  beforeEach(() => {
    ...
    return new Promise((resolve) => {
      const req = indexedDB.deleteDatabase('openwhoop');
...
If any 404: open `openwhoop/dashboard.py`, check the static-file route handler — the existing one may serve only top-level files in `web/`. May need to extend to serve nested `web/js/...`. Fix before proceeding.
...
- Chrome DevTools → Application → IndexedDB → `openwhoop` → `samples` shows rows accumulating
...
**Port from:** `openwhoop/metrics.py:41-110` (`filter_rr`, `rmssd`, `sdnn`, `pnn50`). **Test reference:** `tests/test_metrics.py:12-60`.
...
**Port from:** `openwhoop/metrics.py` (the `strain_score` family). **Test reference:** `tests/test_metrics.py:63-90` (strain tests).
...
**Port from:** `openwhoop/zones.py`. **Test reference:** `tests/test_zones.py`.
...
**Port from:** `openwhoop/sleep.py`. **Test reference:** `tests/test_sleep.py`.
...
**Port from:** `openwhoop/metrics.py` (recovery z-score + 4-component breakdown). Depends on hrv + sleep + strain (for prior-strain component).
...
**Port from:** `openwhoop/workouts.py`. **Test reference:** `tests/test_workouts.py`.
...
Read the list. For each endpoint (e.g. `/api/today`, `/api/sleep`, `/api/recovery`), note the shape of the JSON it returns (read `openwhoop/dashboard.py` to confirm).
...
Port the synthetic-data generator from `openwhoop/cli.py` (`cmd_seed_demo`). Hide behind a query string so it isn't accessible in normal flow.

### Task 4.3: Strip Python API + rename

- [ ] **Step 1:** Rename `openwhoop/dashboard.py` → `openwhoop/static_server.py`.
...
- [ ] **Step 3:** Update `openwhoop/cli.py`'s `dash` command to point at the renamed module.
...
- Add "Where's the data?" → DevTools → Application → IndexedDB → `openwhoop`.
