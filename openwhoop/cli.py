"""Top-level CLI: `openwhoop <command>`.
...
@click.group()
@click.version_option(__version__, prog_name="openwhoop")
...
def cli(ctx: click.Context, verbose: bool, db_path: Optional[str]) -> None:
    """OpenWhoop — unofficial, educational BLE client for the WHOOP 4.0 strap.
...
def seed_demo(ctx: click.Context, days: int, age: int, weight_kg: float, sex: str) -> None:
    ...
    throwaway DB path: `openwhoop --db /tmp/demo.db seed-demo`.
