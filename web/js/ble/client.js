...
export class WhoopClient {
  constructor() {
    ...
    this._gattWriteLock = Promise.resolve();  // serializes charCmd writes (macOS Chrome)
  ...
  async _sendCommand(cmd, payload = new Uint8Array()) {
    ...
    // Serialize all writes: Web Bluetooth allows only one in-flight GATT
    // operation. On macOS Chrome an overlapping write throws "GATT operation
    // already in progress" and can wedge the link, which is why the battery
    // poller racing the post-connect clock/history writes left the session
    // stuck "reconnecting" with only notification-based HR alive.
    const run = this._gattWriteLock.then(async () => {
      const charCmd = this.charCmd;
      if (!charCmd) throw new Error('Not connected');
      ...
      await charCmd.writeValue(frame);
    });
    // Keep the chain alive even if this write rejects, so one failure doesn't
    // permanently block every later command.
    this._gattWriteLock = run.catch(() => {});
    return run;
  }
  ...
  async setAlarm(unixTime) {
    ...
    if (this._family === 'whoop5') {
      await this._sendV5Alarm(unixTime, true);
      return;
    }
    const buf = new Uint8Array(4);
  ...
  async runAlarmNow() {
    if (this._family === 'whoop5') {
      // 5.0 has no fire-now command — schedule the alarm a second out so the
      // strap buzzes effectively immediately.
      await this._sendV5Alarm(Math.floor(Date.now() / 1000) + 1, true);
      return;
    }
  ...
  async disableAlarm() {
    if (this._family === 'whoop5') {
      // 5.0 cancels via the same alarm command with enable=0x00.
      await this._sendV5Alarm(Math.floor(Date.now() / 1000), false);
      return;
    }
  ...
  /**
   * 5.0 (Maverick/Puffin) alarm = vibration. SET_ALARM_TIME (0x42) with a
   * 21-byte payload, verified byte-for-byte against an official-app capture
   * (CRC16+CRC32 match). enable=0x01 arms, 0x00 cancels. The strap buzzes at
   * `unixTime`. Layout after cmd 0x42:
   *   [0x04][enable][unix_ts:u32 LE][00 00 2F 98 00*8 07 1E 00]
   */
  async _sendV5Alarm(unixTime, enable) {
    const buf = new Uint8Array(21);
    buf[0] = 0x04;
    buf[1] = enable ? 0x01 : 0x00;
    writeU32LE(buf, 2, unixTime);
    buf.set([0x00, 0x00, 0x2f, 0x98, 0, 0, 0, 0, 0, 0, 0, 0, 0x07, 0x1e, 0x00], 6);
    await this._sendCommand(CommandNumber.SET_ALARM_TIME, buf);
  }
