/* openwhoop v0.2 — dashboard front-end */
...
async function fetchJSON(url, opts) {
  ...
  if (window.openwhoopApi) {
    const shimResult = await window.openwhoopApi.handle(url, opts);
...
async function loadOverview() {
  ...
  if (welcomeEl) welcomeEl.textContent = `${greeting}, user`;
...
async function exportDailyMetricsCsv() {
  ...
  a.href = url;
  a.download = `openwhoop-daily-metrics-${stamp}.csv`;
