...
describe('WhoopClient mocked BLE', () => {
  ...
  describe('command helpers send the right cmd byte', () => {
    ...
    beforeEach(async () => {
      // Suppress the fire-and-forget post-connect flow so its background
      // clock/history/realtime writes don't leak into these command-byte slices.
      vi.spyOn(client, '_postConnectFlow').mockResolvedValue();
      await client.connectToDevice(device);
    ...
    it('serializes concurrent writes (no overlapping GATT op)', async () => {
      // macOS Chrome wedges on overlapping writeValue. Make each write hang
      // until released and assert at most one is ever in flight.
      let inFlight = 0, maxInFlight = 0;
      const releases = [];
      device._chars.cmd.writeValue = vi.fn(() => {
        inFlight++; maxInFlight = Math.max(maxInFlight, inFlight);
        return new Promise(res => releases.push(() => { inFlight--; res(); }));
      });
      const p = Promise.all([client.getBatteryLevel(), client.runHaptics(), client.sendHello()]);
      // Drain the chain: release writes one at a time as they queue up.
      while (releases.length < 3 || inFlight > 0) {
